...
public class AlfrescoCMISService implements IAlfrescoCMISService {
	...
	@Override
	public ReturnMessageDto uploadDocument(Integer idAgent, EaeFinalizationDto eaeFinalizationDto,
		...
		String pathAgentEae = CmisUtils.getPathEAE(
    			eae.getEaeEvalue().getIdAgent(), agentDto.getDisplayNom(), agentDto.getDisplayPrenom());
	    ...
	    while(!isCreated) {
		    ...
		    ContentStream contentStream = new ContentStreamImpl(
		    		name, 
		    		BigInteger.valueOf(eaeFinalizationDto.getbFile().length), 
...
//		props.put("mairie:commentaire", "");
	...
	private String getDescriptionEae(String annee, Agent agent) {
		...
		if(null != agent
				&& null != annee) {
			description = "EAE de l'agent " + agent.getDisplayPrenom() 
					+ " " + agent.getDisplayNom() + " pour l'année " + annee;
		}
